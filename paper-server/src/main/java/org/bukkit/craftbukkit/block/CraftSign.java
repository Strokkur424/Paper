...
public class CraftSign<T extends SignBlockEntity> extends CraftBlockEntityState<T> implements Sign {
    ...
    public SignSide getTargetSide(Player player) {
        ...
        if (this.getSnapshot().isFacingFrontText(((CraftPlayer) player).getHandle().getX(), ((CraftPlayer) player).getHandle().getZ())) {
            return this.front;
        }
    ...
    public static void openSign(Sign sign, Player player, Side side) {
        ...
        ((CraftPlayer) player).getHandle().openTextEdit(blockEntity, Side.FRONT == side ? SignTextSlot.FRONT : SignTextSlot.BACK);
    }
